...
- `packages/shared`: Shared ID helpers and Green Leaf Book calculation logic, including automatic arrears carry-forward.
...
- Negative Green Leaf Book balances continue into the next month as arrears for the same supplier, including suppliers without a recorded payment. Desktop and backend use the shared automatic arrears calculation and the backend loads previous-month calculation inputs when rendering a selected month.

...
- Web balance and advance signals are stored separately from real desktop payments/advances. Balance signals preserve the signalled amount, payment done date, optional comment, signalling user, signalling timestamp, and read metadata (`read_at`, `read_by_user_id`, `read_by_display_name`) used by office users/admins to clear handled signals from the default web listings. Directors can update/delete only signals they originally added, while super admins can update/delete any signal. Advance signals also preserve scope, supplier/line target, effective month, date given, suggested amount, and per-supplier suggestion breakdown.
- Web audit records are stored in `web_audit_log` for admin-only review of web app actions such as login/logout, managed user changes, balance/advance signal create/update/delete actions, and signal read marking. Audit snapshots exclude passwords, hashes, tokens, and authorization values.
...
- Fertilizer bag types are stored by name, type, and bag weight. Received fertilizer stock lots store received date, fertilizer type, per-bag price, and bags received. Fertilizer issues are stored by supplier, given date, selected stock/type where available, bags issued, calculated kg, calculated total rupee value, split count, and effective month or months. Monthly fertilizer installments are generated from those issues and drive the Green Leaf Book fertilizer deduction for each selected month.
- Made tea packet types are stored by name, weight, and price. Made tea packet deductions are stored by supplier, given date, selected packet type where available, snapshot packet name/weight/price, packet count, total amount, and effective month. They are included in Green Leaf Book totals and the desktop book table for the selected month only, and later edits to packet type prices do not change already issued rows.

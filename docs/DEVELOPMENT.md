...
Use Made Tea Packets to register packet types with name, weight, and price, edit registered packet details when future prices change, and issue selected packet types to suppliers for deduction in a selected effective Green Leaf Book month.
Advance, fertilizer issue, received fertilizer, and made tea packet issue forms use type-in suggestion fields for long supplier and product/stock choices while preserving hidden record IDs in the submitted payload.
The web app shows lightweight loading spinners during authentication restore, page loading, refresh, save, edit, and mark-read actions. The admin-only Audit Trail shows snapshots as readable field-level before/after rows, and Advance Signal action buttons follow consistent sizing with distinct colors for each action.

...
- Audit Reports shows an append-only office action trail for creations, updates, checkbox/status changes, staging posts, price override batches, and payment recording. Viewing-only operations are not logged, and sensitive values such as passwords are excluded from before/after details.

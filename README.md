...
- The desktop-to-web sync uses configured `BACKEND_URL` and `CLOUD_SYNC_TOKEN` values, not human-entered web credentials. Desktop admin users can save the hosted backend URL from the Sync to Web App screen in deployed installs; non-admin office users do not see that setup UI.
...
- Fertilizer has collapsed desktop sections for registering fertilizer bag types, recording received stock lots, issuing stock lots to suppliers, and reviewing current stock balances. Issuing fertilizer records supplier, date given, selected stock lot, bags issued, calculated kg/value, one-month or two-month repayment split, and the effective month or months for Green Leaf Book deductions.
- Made Tea Packets has collapsed desktop sections for registering packet types by name, weight, and price, editing those registered details when prices change, and issuing selected packet types to suppliers. Issued packet records keep their saved price/total, packet count, date given, and effective month for Green Leaf Book deductions even if the registered packet price is edited later.
- Office users can open Pair Tablet to show a QR code that stores the current desktop sync URL on a tablet.
- Office users can open Sync to Web App below Green Leaf Book and press Sync now. Desktop admin users also get a setup form there to save the hosted backend URL for deployed installs and can update the cloud sync token if needed; non-admin office users only see the sync action and run history. Normal daily syncs send posted collection entries changed after the last successful sync and resend Green Leaf Book adjustment/reference data idempotently, including monthly settings, supplier special prices, advances, fertilizer installments, made tea packets, supplier payments, arrears, and month closures. A full sync checkbox is available for setup/recovery, and a Green Leaf Book-only checkbox skips office-user account sync for daily use. Sync run history is paginated, filterable, and pruned after 180 days while keeping at least the latest 500 runs.

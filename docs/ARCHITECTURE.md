...
- Made tea packet types are registered on desktop by name, weight, and current price, and can be edited by office users or admins when prices change. Made tea packet issues preserve supplier, given date, selected packet type where available, snapshot packet name/weight/price, packet count, total rupee value, and effective month so later type-price edits do not alter existing issued packet deductions. The Green Leaf Book deducts only made tea packet records for the selected month.
...
- Sidebar: Green Leaf Book, Balances, Advances, plus Directors for super admins and directors, Office Users for super admins, directors, and office users, and Audit Trail for super admins.
- Login: desktop-aligned two-column portal login with branded copy and credential form.
- Content: monthly green leaf book viewing, balance-transfer signal viewing/marking, advance signal viewing/marking, admin-only web audit review, managed web-user administration, and profile details. Managed user create/edit/activate/deactivate actions are limited to super admins; directors have view-only access to directors and office users, and office users have view-only access to office users. Directors/admins create balance and advance signals; directors can edit/delete only records they added, super admins can edit/delete any signal, and office users/admins can mark those signals as read. Signal listings include read rows by default, can hide read rows with a checkbox, sort latest-first, and support filters and pagination. Balance signal groups are collapsible and start collapsed. Web forms and page refresh actions show lightweight loading spinners while data is being loaded or saved. Advance Signal action buttons use consistent sizing with distinct colors for read, edit, save, cancel, and delete actions. Audit Trail snapshot details render as readable field-level before/after change rows for admin review.

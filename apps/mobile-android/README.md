...
- Offline collection record creation, receipt preview, edit-before-upload flow, tablet saved/printed timestamp capture as ISO instants, print-only reprints from Recorded Data, and upload to desktop staging review.
...
7. Enter bags and gross weight, then save and preview the receipt. The printed receipt shows Kudamalana Tea Factory, Green Leaf Collection, a shortened receipt ID, saved time, and printed time. Tablet saved/printed instants are stored for sync, while receipt text displays the device-local time.
